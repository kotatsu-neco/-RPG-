...
async function boot() {
  state.map = await fetch("assets/maps/prototype/one_screen_map.json?v=0.9").then((res) => res.json());
  ...
  const colpan = [
    "assets/sprites/colpan/spr_colpan_proto_seq01_frame01.png?v=0.9",
    "assets/sprites/colpan/spr_colpan_proto_seq01_frame02.png?v=0.9",
    "assets/sprites/colpan/spr_colpan_proto_seq01_frame03.png?v=0.9",
    "assets/sprites/colpan/spr_colpan_proto_seq01_frame04.png?v=0.9",
  ];

  const land = [
    "assets/sprites/land/spr_land_large_dog_right_01.png?v=0.9",
    "assets/sprites/land/spr_land_large_dog_right_02.png?v=0.9",
    "assets/sprites/land/spr_land_large_dog_right_03.png?v=0.9",
    "assets/sprites/land/spr_land_large_dog_right_04.png?v=0.9",
  ];
  ...
  for (const npc of state.map.npcs) {
    state.images[npc.id] = await loadImage(`${npc.sprite}?v=0.9`);
  }
...
function movePlayer(dir) {
  if (state.dialogueOpen || state.noticeOpen) return;
...
function syncOverlayState() {
  ...
  document.body.classList.toggle("notice-open", state.noticeOpen);

  const shouldShowWideTap = (state.dialogueOpen && !isChoiceOpen()) || state.noticeOpen;

  dialogTapCatcher.classList.toggle("hidden", !shouldShowWideTap);
  noticeLayer.classList.toggle("hidden", !state.noticeOpen);

  // CSS側の状態ルールを優先するため、touchControlsのhiddenクラスは基本的に使わない。
  touchControls.classList.remove("hidden");
}

function checkTileTriggers() {
  if (state.scene !== "village") return;
  if (!state.map.triggers || state.dialogueOpen || state.noticeOpen) return;
...
function drawObjects() {
  ...
  ctx.fillStyle = "rgba(198, 163, 95, 0.18)";
  ctx.fillRect(8 * TILE, 8 * TILE, TILE * 4, TILE * 2);
  ctx.strokeStyle = "rgba(198, 163, 95, 0.65)";
  ctx.strokeRect(8 * TILE + 1, 8 * TILE + 1, TILE * 4 - 2, TILE * 2 - 2);
}
